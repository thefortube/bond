...
var output = 'fortube_migrations_init.log';
...
module.exports = async function (deployer, network) {
    ...
    for (let i = 0; i < deposit_tokens.length; ++i) {
        ...
        let deposit_multiple = BigNumber(deposit_token.deposit_multiple).multipliedBy(1e18).dividedBy(100).toFixed();
    ...
    var deployed_ACL = await ACL.deployed();
    var deployed_ACL_address = deployed_ACL.address;
    
    await deployed_ACL.enableany(BondFactory.address, Router.address);
    await deployed_ACL.enableany(BondFactory.address, deployed_ACL_address);
    
    promises = [];
    for(let i = 0; i < env.oracle_admin.length; ++i) {
        logger.write("deployed_ACL.enableany: " + env.oracle_admin[i] + "Oracle.address" + Oracle.address + "\n");
        promises.push(deployed_ACL.enableany(env.oracle_admin[i], Oracle.address));
    }
    ...
    for(let i = 0; i < env.pra_admin.length; ++i) {
        logger.write(util.format("deployed_ACL.enable(%s, %s, %s)" + env.pra_admin[i], PRA.address, set_sig) + "\n");
        promises.push(deployed_ACL.enable(env.pra_admin[i], PRA.address, set_sig));  
    }
